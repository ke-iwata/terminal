...
pub struct TermModes {
    ...
    /// CSI ?2004 - bracketed paste: the application wants pasted text
    /// wrapped in ESC[200~ / ESC[201~ so it can tell a paste apart from
...
pub struct Term {
    ...
    /// loop drains them with `take_responses` after each `advance`.
    responses: Vec<u8>,
}
...
impl Term {
    pub fn new(cols: usize, rows: usize, scrollback_limit: usize) -> Self {
        ...
        Term {
            ...
            // No scrollback for the alternate screen: full-screen apps
            // manage their own scrolling, the wheel is disabled on the
            // alt screen, and lines vim scrolls by would otherwise pile
            // up invisibly to the scrollback cap for nothing.
    ...
    /// Update the scrollback cap (primary screen only -- the alternate
    /// screen never keeps scrollback), trimming immediately if it shrank.
    /// Used when settings are changed live from the settings window.
    pub fn set_scrollback_limit(&mut self, limit: usize) {
        self.scrollback_limit = limit;
        self.grid.set_scrollback_limit(limit);
    }

    /// Drain any query responses (DSR/DA) generated by the bytes fed to
    /// `advance` since the last drain. The caller writes them back to the
    /// pty -- applications that ask "where is the cursor?" (CSI 6n) block
    ...
    fn erase_in_display(&mut self, mode: u16) {
        ...
        match mode {
            ...
            3 => {
                // ED 3 (xterm "Erase Saved Lines"): clears scrollback,
                // not the visible screen -- the modern `clear` command
                // sends CSI H, CSI 2J, CSI 3J as a trio expecting exactly
                // this split.
    ...
    fn reset(&mut self) {
        ...
        self.alt_grid = Grid::new(cols, rows, 0);
...
mod tests {
    ...
    fn dsr_reports_cursor_position() {
        let mut term = Term::new(10, 5, 10_000);
        term.advance(b"\x1b[3;5H\x1b[6n");
        assert_eq!(term.take_responses(), b"\x1b[3;5R".to_vec());
        assert!(term.take_responses().is_empty(), "drained once, gone");
    }

    #[test]
    fn da1_identifies_the_terminal() {
        let mut term = Term::new(10, 5, 10_000);
    ...
    #[test]
    fn ed3_clears_scrollback_but_not_the_screen() {
        let mut term = Term::new(5, 2, 10_000);
        term.advance(b"11111\r\n22222\r\n33333"); // "11111" scrolled off
        assert_eq!(term.grid().scrollback.len(), 1);
        term.advance(b"\x1b[3J");
        assert_eq!(term.grid().scrollback.len(), 0);
        assert_eq!(term.grid().cell(0, 0).c, '2', "visible screen untouched");
    }

    #[test]
    fn alt_screen_keeps_no_scrollback() {
        let mut term = Term::new(5, 2, 10_000);
